...
public class PoisonRingObject : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        gameObject.TweenSpriteRendererColor(Color.white - new Color(0f, 0f, 0f, 0.25f), 0.5f).SetEaseCubicInOut().SetPingPong().SetInfinite();
        ...
        while (elapsedTime <= duration)
        {
            ...
            {
                // unit might've died in-between colliding and damaging
                if (unit != null)
                {
                    unit.health -= damagePerSecond / tickRate;
                }
            }
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (unit.ownership == UnitOwnership.Roman)
        {
            units.Add(unit);
        }
    }
