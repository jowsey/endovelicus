...
{
    public Summon(int level = 1) : base(Icons.ResolveIcon(Icons.SwordBrandish), level, 4)
    {
        NewLevelValue("melees", new List<float> { 1, 2, 4, 6 });
        NewLevelValue("archers", new List<float> { 2, 3, 5, 7 });

        NewLevelValue("devotionCost", new List<float> { 18, 30, 54, 78 });
        NewLevelValue("cooldown", new List<float> { 10, 10, 12, 12, 14 });
    }

    public override string GetDescription() =>
        $"Summon followers to fight for you.\nSummons {FormatLevelValue("archers")} archer(s) and {FormatLevelValue("melees")} melee unit(s).";
    ...
    public override IEnumerator CastEffect()
    {
        yield return GetMouseClickPosition(clickPosition =>
        {
            ...
            nearestVillage.PlaceSummonOrder(new SummonOrder(
                ...
                {
                    {UnitClass.Melee, (int) LevelValue("melees")},
                    {UnitClass.Archer, (int) LevelValue("archers")}
                },
        ...
        foreach (var unitSet in summonOrder.unitSets)
        {
            for (var i = 0; i < unitSet.Value; i++)
            {
                ...
                if (summonOrder.ownership == UnitOwnership.Friendly)
                {
                    ...
                    StatsManager.stats[StatType.UnitsSummoned]++;
                }
